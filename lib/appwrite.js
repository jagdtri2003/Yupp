...
export async function getAllPosts() {
  try {
    ...
    FileSystem.writeAsStringAsync(path,JSON.stringify(posts.documents));
...
export async function likePost(postId, userId) {
  try {
    ...
    if (!post.LikedBy.some(liker => liker.$id === userId)) {
...
export async function unlikePost(postId, userId) {
  try {
    ...
    const likedByUser = post.LikedBy.some(liker => liker.$id === userId);

    if (!likedByUser) {
      throw new Error("User has not liked this post.");
    }

    console.log("New UnLike !")
    // Remove the user's object from the LikedBy array
    const updatedLikedBy = post.LikedBy.filter((liker) => liker.$id !== userId);

    const idS = updatedLikedBy.map((like) => like.$id);
...
export const getLikesByUser = async (userId, postId, setLikesCount, setLiked) => {
  try {
    ...
    const isLiked = LikedBy.some((like) => like.$id === userId);
...
export const subscribeToLikeChanges = (postId, userId, setLikesCount, setLiked) => {
  return client.subscribe(
    ...
    (event) => {
      ...
      console.log(event.payload.postId);
...
export const deletePost = (postId)=>{
  return databases.deleteDocument(appwriteConfig.databaseId, appwriteConfig.userPostCollectionId, postId);
}
